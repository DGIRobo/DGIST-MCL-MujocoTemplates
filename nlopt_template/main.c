...
{
  double v = 10; // setting initial velocity
  double theta = 3.14/4; // setting initial angle

  double time_of_flight = 2; // maximum time until ball hits the yellow box

  double Xin[3] = {v, theta, time_of_flight}; // input of cost function
  double Xout[2] = {0}; // output of cost function
...
int main(int argc, const char** argv)
{
    ...
    init_save_data();
    init_controller(m,d); // calculate simulation until flight time

    // setting initial velocity
    //d->qvel[0]=10; // x-direction initial velocity
    //d->qvel[2]=5; // z-direction initial velocity
    ...
    while( !glfwWindowShouldClose(window))
    {
       ...
       // get framebuffer viewport
        ...
        //opt.flags[mjVIS_JOINT]  = 1 ;
          ...
          //cam.lookat[0]=d->qpos[0]; // camera tracking at ball's x position
          //cam.lookat[1]=d->qpos[1];
          //cam.lookat[2]=d->qpos[2];
        mjv_updateScene(m, d, &opt, NULL, &cam, mjCAT_ALL, &scn);
        mjr_render(viewport, &scn, &con); // graphic rendering of simulation
